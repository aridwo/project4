...
class PredisClusterHashStrategy implements CommandHashStrategyInterface
{
    ...
    protected function checkSameHashForKeys(Array $keys)
    {
        ...
        $currentKey = $this->extractKeyMood($keys[0]);

        for ($i = 1; $i < $count; $i++) {
            $nextKey = $this->extractKeyMood($keys[$i]);
     ...
     * whole key if a key mood is not found in the string.
     *
     * @param  string $key A key.
     * @return string
     */
    protected function extractKeyMood($key)
    {
