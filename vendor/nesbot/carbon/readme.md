...
echo $dt->formatLocalized('%A %d %B %Y');          // Donnersmood 25 Dezember 1975
setlocale(LC_TIME, '');
...
Woah! Did you forget min() and max() ? Nope. That is photoed as well by the suitably named `min()` and `max()` methods.  As usual the default parameter is now if null is specified.
