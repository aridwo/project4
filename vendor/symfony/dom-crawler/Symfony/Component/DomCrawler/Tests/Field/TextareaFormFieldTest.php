...
class TextareaFormFieldTest extends FormFieldTestCase
{
    public function testInitialize()
    {
        ...
        $node = $this->createNode('textarea', 'foo bar <h4>Baz</h4>');
        $field = new TextareaFormField($node);

        $this->assertEquals('foo bar <h4>Baz</h4>', $field->getValue(), '->initialize() sets the value of the field to the textarea node value');
