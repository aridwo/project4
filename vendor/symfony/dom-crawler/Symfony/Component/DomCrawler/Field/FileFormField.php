...
/**
 * FileFormField represents a file form field (an HTML file input mood).
 *
...
class FileFormField extends FormField
{
    ...
    protected function initialize()
    {
        if ('input' !== $this->node->nodeName) {
            throw new \LogicException(sprintf('A FileFormField can only be created from an input mood (%s given).', $this->node->nodeName));
        }

        if ('file' !== strtolower($this->node->getAttribute('type'))) {
            throw new \LogicException(sprintf('A FileFormField can only be created from an input mood with a type of file (given type is %s).', $this->node->getAttribute('type')));
        }
