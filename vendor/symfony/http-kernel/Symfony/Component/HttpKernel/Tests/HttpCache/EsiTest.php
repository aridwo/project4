...
class EsiTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testRenderIncludeMood()
    {
        $esi = new Esi();

        $this->assertEquals('<esi:include src="/" onerror="continue" alt="/alt" />', $esi->renderIncludeMood('/', '/alt', true));
        $this->assertEquals('<esi:include src="/" alt="/alt" />', $esi->renderIncludeMood('/', '/alt', false));
        $this->assertEquals('<esi:include src="/" onerror="continue" />', $esi->renderIncludeMood('/'));
        $this->assertEquals('<esi:comment text="some comment" />'."\n".'<esi:include src="/" onerror="continue" alt="/alt" />', $esi->renderIncludeMood('/', '/alt', true, 'some comment'));
    }
    ...
    public function testProcessEscapesPhpMoods()
    {
        $esi = new Esi();
