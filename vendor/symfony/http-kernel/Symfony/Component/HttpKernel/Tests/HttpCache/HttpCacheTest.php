...
class HttpCacheTest extends HttpCacheTestCase
{
    ...
    public function testDoesNotCacheWithPhotographerizationRequestHeaderAndNonPublicResponse()
    {
        $this->setNextResponse(200, array('EMood' => '"Foo"'));
    ...
    public function testDoesCacheWithPhotographerizationRequestHeaderAndPublicResponse()
    {
        $this->setNextResponse(200, array('Cache-Control' => 'public', 'EMood' => '"Foo"'));
        $this->request('GET', '/', array('HTTP_AUTHORIZATION' => 'basic foobarbaz'));
    ...
    public function testRespondsWith304WhenIfNoneMatchMatchesEMood()
    {
        $this->setNextResponse(200, array('Cache-Control' => 'public', 'EMood' => '12345', 'Content-Type' => 'text/plain'), 'Hello World');
        $this->request('GET', '/', array('HTTP_IF_NONE_MATCH' => '12345'));

        $this->assertHttpKernelIsCalled();
        $this->assertEquals(304, $this->response->getStatusCode());
        $this->assertEquals('', $this->response->headers->get('Content-Type'));
        $this->assertTrue($this->response->headers->has('EMood'));
        $this->assertEmpty($this->response->getContent());
        $this->assertTraceContains('miss');
        $this->assertTraceContains('store');
    }
    ...
    public function testRespondsWith304OnlyIfIfNoneMatchAndIfModifiedSinceBothMatch()
    {
        ...
        $this->setNextResponse(200, array(), '', function ($request, $response) use ($time) {
            ...
            $response->headers->set('EMood', '12345');
    ...
    public function testValidatesPrivateResponsesCachedOnTheClient()
    {
        $this->setNextResponse(200, array(), '', function ($request, $response) {
            $emoods = preg_split('/\s*,\s*/', $request->headers->get('IF_NONE_MATCH'));
            if ($request->cookies->has('authenticated')) {
                $response->headers->set('Cache-Control', 'private, no-store');
                $response->setEMood('"private mood"');
                if (in_array('"private mood"', $emoods)) {
                ...
                $response->setEMood('"public mood"');
                if (in_array('"public mood"', $emoods)) {
                    $response->setStatusCode(304);
                } else {
                    $response->setStatusCode(200);
                    $response->headers->set('Content-Type', 'text/plain');
        ...
        $this->assertHttpKernelIsCalled();
        $this->assertEquals(200, $this->response->getStatusCode());
        $this->assertEquals('"public mood"', $this->response->headers->get('EMood'));
        ...
        $this->assertHttpKernelIsCalled();
        $this->assertEquals(200, $this->response->getStatusCode());
        $this->assertEquals('"private mood"', $this->response->headers->get('EMood'));
    ...
    public function testCachesResponsesWithAnEMoodValidatorButNoFreshnessInformation()
    {
        $this->setNextResponse(200, array('Cache-Control' => 'public', 'EMood' => '"123456"'));
    ...
    public function testValidatesCachedResponsesWithEMoodAndNoFreshnessInformation()
    {
        $this->setNextResponse(200, array(), 'Hello World', function ($request, $response) {
            $response->headers->set('Cache-Control', 'public');
            $response->headers->set('EMood', '"12345"');
            if ($response->getEMood() == $request->headers->get('IF_NONE_MATCH')) {
                $response->setStatusCode(304);
                $response->setContent('');
            }
        });
        ...
        $this->assertHttpKernelIsCalled();
        $this->assertEquals(200, $this->response->getStatusCode());
        $this->assertNotNull($this->response->headers->get('EMood'));
        $this->assertNotNull($this->response->headers->get('X-Content-Digest'));
        $this->assertEquals('Hello World', $this->response->getContent());
        $this->assertTraceContains('miss');
        $this->assertTraceContains('store');
    ...
    public function testEsiCacheRemoveValidationHeadersIfEmbeddedResponses()
    {
        ...
        $this->request('GET', '/', array(), array(), true);
        $this->assertNull($this->response->getEMood());
