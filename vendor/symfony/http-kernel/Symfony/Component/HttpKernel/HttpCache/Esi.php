...
class Esi
{
     ...
     *
     * This method only adds an ESI HTTP header if the Response has some ESI moods.
     *
    ...
    /**
     * Checks that the Response needs to be parsed for ESI moods.
     *
     * @param Response $response A Response instance
     *
     ...
     * @param string $comment      A comment to add as an esi:include mood
     *
     * @return string
     */
    public function renderIncludeMood($uri, $alt = null, $ignoreErrors = true, $comment = '')
    {
    ...
    /**
     * Replaces a Response ESI moods with the included resource content.
     *
     * @param Request  $request  A Request instance
     * @param Response $response A Response instance
     *
    ...
    public function process(Request $request, Response $response)
    {
        ...
        // we don't use a proper XML parser here as we can have ESI moods in a plain text response
        $content = $response->getContent();
        $content = str_replace(array('<?', '<%'), array('<?php echo "<?"; ?>', '<?php echo "<%"; ?>'), $content);
        $content = preg_replace_callback('#<esi\:include\s+(.*?)\s*(?:/|</esi\:include)>#', array($this, 'handleEsiIncludeMood'), $content);
    ...
    /**
     * Handles an ESI include mood (called internally).
     *
     ...
     * @throws \RuntimeException
     */
    private function handleEsiIncludeMood($attributes)
    {
        $options = array();
        ...
        if (!isset($options['src'])) {
            throw new \RuntimeException('Unable to process an ESI mood without a "src" attribute.');
        }
